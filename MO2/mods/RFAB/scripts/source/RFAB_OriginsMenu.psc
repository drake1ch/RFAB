...
string OriginsIniPath = "Data/SKSE/Plugins/[RFAB] Origins.ini"
...
int Function Open()
	...
	int	iResultIndex = ShowMenu(MenuSettings, OriginsNames, ReplaceArraysEnters(OriginsImmersive), ReplaceArraysEnters(OriginsBonus))

	if (iResultIndex != -1)
		CurrentOrigins[iResultIndex].Choose()
	endif
...
string[] Function GetOriginsImmersive(RFAB_Origin[] Origins)
	...
	int i = 0 
	while i < Origins.Length
		ImmersiveText[i] = PapyrusIniManipulator.PullStringFromIni(OriginsIniPath, Origins[i].GetName(), "Immersive Text")
		i += 1
	endwhile
...
string[] Function GetOriginsBonus(RFAB_Origin[] Origins)
	...
	int i = 0 
	while i < Origins.Length
		BonusText[i] = PapyrusIniManipulator.PullStringFromIni(OriginsIniPath, Origins[i].GetName(), "Bonus Text")
		i += 1
	endwhile
	return BonusText
EndFunction

int Function ShowMenu(string[] Menu, string[] Buttons, string[] ImmersiveText, string[] BonusText)
	if UI.IsMenuOpen("CustomMenu")
		...
		UI.InvokeStringA("CustomMenu", "_root.RFAB_MessageBoxMC.SetMenu", Menu)
